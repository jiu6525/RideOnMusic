  ...
  axios
    ...
    .then((res) => {
      if (res.data.tracks.items<10)  $state.complete()
        ...
        searchResults.value.push(...res.data.tracks.items.map((track) => ({
  ...
  <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <input class="form-control w-50" type="search" placeholder="Search Songs/Artists" v-model="searchword"/>
        <!-- <h1 class="modal-title fs-5" id="exampleModalLabel">노래 검색</h1> -->
        <!-- <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button> -->
      </div>
      <div class="modal-body modalBody" v-if="searchword">
        <TrackSearchResult class="result"
